...
final class GitLabSwift_DiscussionsTests: XCTestCase {
    ...
    public func test_detail() async throws {
        let result = await catchErrors {
            let response = try await gitlab.discussions.get(
                discussionID: "2d2b6632ccb10913fdf6e220eca765712511e9c1",
                issueID: 3010,
                project: .id(1097)
            )
    ...
    public func test_createThread() async throws {
        let result = await catchErrors {
            let response = try await gitlab.discussions.create(issueID: 3010, body: "New thread message", project: .id(1097))
    ...
    public func test_listSnippetDiscussions() async throws {
        let result = await catchErrors {
            let response = try await gitlab.discussions.list(snippetID: 372, project: .id(2008))
    ...
    public func test_getSnippetDiscussionItem() async throws {
        let result = await catchErrors {
            let response = try await gitlab.discussions.get(584630, discussionID: 372, project: .id(2008))
